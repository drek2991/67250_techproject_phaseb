...
if (document.getElementById("checkoutForm")) {
    ...
    if (dateParam) {
        ...
        if (!isNaN(d)) {
            document.getElementById("visitDate").value = d.toISOString().split("T")[0];
        }
    }
    ...
    document.getElementById("placeOrder").addEventListener("click", function () {
        ...
        if (valid) {
            const total = qty * 18;
            window.location.href = "confirmation.html?total=" + total + "&qty=" + qty + "&date=" + encodeURIComponent(date) + "&type=" + encodeURIComponent(type);
        }
    });
}
...
if (document.getElementById("orderSummary")) {
    ...
    document.getElementById("confirmType").textContent = type ? type.charAt(0).toUpperCase() + type.slice(1) : "N/A";
    document.getElementById("confirmQty").textContent = qty || "N/A";
    document.getElementById("confirmTotal").textContent = total ? "$" + total : "N/A";
}
...
if (document.getElementById("gallery")) {
    ...
    const alts = [
        "Exhibition gallery image 1",
        "Exhibition gallery image 2",
        "Exhibition gallery image 3"
    ];
    let current = 0;

    function changeSlide(direction) {
        current = (current + direction + images.length) % images.length;
        document.getElementById("slideImg").src = images[current];
        document.getElementById("slideImg").alt = alts[current];
        document.getElementById("slideCounter").textContent = (current + 1) + " / " + images.length;
    }
}
...
if (document.getElementById("donateForm")) {
    ...
    if (amountParam) {
        ...
        if (amountSelect) {
            amountSelect.value = amountParam;
        }
    }

    // Auto donation total calculation
    document.getElementById("donationAmount").addEventListener("change", updateDonationTotal);

    function updateDonationTotal() {
        const amount = parseInt(document.getElementById("donationAmount").value) || 0;
        document.getElementById("donationDisplay").textContent = "Total: $" + amount;
    }
    ...
    document.getElementById("donateNow").addEventListener("click", function () {
        ...
        if (!amount) {
            document.getElementById("donationAmountError").textContent = "Please select a donation amount.";
            valid = false;
        }
        ...
        if (valid) {
            window.location.href = "donation-confirmation.html?amount=" + encodeURIComponent(amount);
        }
    });
}

function displayDonationConfirmation() {
    const params = new URLSearchParams(window.location.search);
    const amount = params.get("amount");

    if (document.getElementById("confirmAmount")) {
        document.getElementById("confirmAmount").textContent = amount ? "$" + amount : "$0";
    }
}